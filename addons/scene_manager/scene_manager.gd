...
func _set_current_scene() -> void:
	...
	if _current_scene == "":
		if !_current_scene_is_ignored(scene_file_path):
			push_warning("Scene Manager Warning: loaded scene is not defined in scene manager tool, to fix this, on Scene Manager UI panel, just once click on refresh and then save buttons respectively.")
		else:
