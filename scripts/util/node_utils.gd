...
## 功能说明：
## - 提供静态节点工具函数
## - 包含节点查找、添加、删除、遍历等常用操作
## - 支持递归操作和自定义条件查找
## 
## 主要函数：
## - get_grandparent(): 获取指定层级的父节点
## - remove_child*(): 各种删除子节点的方法
## - add_child*(): 各种添加子节点的方法
## - apply_function_to_all_children(): 递归应用函数到所有子节点
## - find_descendant(): 递归查找符合条件的后代节点
## 
## 使用场景：
## - 动态构建UI树
## - 批量操作节点
