...
## 
## 功能说明：
## - [GameSaveData]中的变量会在调用[select_save_file]后自动保存和加载
## - [GameSaveData]中的变量会在调用[exit_save_file]后暂停更新
## - [MetaSaveData]中的变量用于存储存档文件的元数据（如文件名、游戏时长等），而非实际游戏数据
## 
## 架构设计：
## - 此节点包含多个[SaveData]类型的子节点
## - 每个[SaveData]子节点代表存档文件的一个部分，保存不同类型的数据
## - 默认项目包含两个存档节点：[GameSaveData]（游戏数据）和[MetaSaveData]（元数据）
## - 如果子节点是[SaveData.is_metadata()]，其数据会在选择存档文件前加载
## - 一旦选择了存档文件，每个[SaveData]子节点都会自动保存其变量中的数据
## 
## 文件系统结构：
## 存档文件在文件系统中以JSON格式保存
## 存档文件系统结构示例：
...
func _system_write_from(index: int, save_data: SaveData) -> void:
	...
	LogWrapper.debug(self, "System write: %s from %s" % [path, "_system_write_from"])
